...
Accounts.registerLoginHandler(loginRequest => {
  ...
  // in user
  const res = Accounts.updateOrCreateUserFromExternalService('discourse', {
  ...
  // We could also use the onExternalLogin() hook, this would save us a db
  // write. However, it would also require a hack, because this hook doesn't
  // receive the service in case of user creation (no immediate way to now
  // the user is being created following a discourse sso login attempt)
  Meteor.users.update(res.userId, { $set: { username: payload.username } })
